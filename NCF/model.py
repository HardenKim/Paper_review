...
class NCF(Model):
    def __init__(self, feature_columns, hidden_units=None, dropout=0.2, activation='relu', embed_reg=1e-6, **kwargs):
        ...
        self.mlp_item_embedding = Embedding(input_dim=self.item_fea_col['feat_num'],
                                           ...
                                           embeddings_regularizer=l2(embed_reg))
        # dnn
        self.dnn = DNN(hidden_units=hidden_units, activation=activation, dnn_dropout=dropout)
...
def test_model():
    user_features = {'feat': 'user_id', 'feat_num': 10, 'embed_dim': 8}
    item_features = {'feat': 'item_id', 'feat_num': 20, 'embed_dim': 8}
